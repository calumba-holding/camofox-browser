...
/**
 * Read the plugins list from camofox.config.json.
 * Returns null if no config or no plugins key (load all).
 * Returns string[] if plugins are explicitly listed.
 */
...
export async function loadPlugins(app, ctx) {
  ...
  const allowList = readPluginList();
